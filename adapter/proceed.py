...
def add_adapters_(parent_module: nn.Module, args, top_level=True):
    for name, module in parent_module.named_children():
        conv1d_types = (nn.Conv1d, nn.Linear)
        if transformers is not None:
            conv1d_types = conv1d_types + (transformers.Conv1D,)
        
        if args.tune_mode == 'all_down_up' and isinstance(module, conv1d_types + (nn.LayerNorm, nn.BatchNorm1d)):
            down_up.add_down_up_(parent_module, name, freeze_weight=args.freeze, merge_weights=args.merge_weights,)
        elif args.tune_mode == 'down_up' and isinstance(module, conv1d_types):
            down_up.add_down_up_(parent_module, name, freeze_weight=args.freeze, merge_weights=args.merge_weights,)
        else:
