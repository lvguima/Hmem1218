...
def add_down_up_(parent_module: nn.Module, module_name: str, freeze_weight: bool,
    ...
    elif transformers is not None and isinstance(old_module, transformers.Conv1D):
...
class TFMConv1D(Down_Up, ssf.AttnConv1D):
    def __init__(
        ...
        if transformers is None:
            raise ImportError("transformers library is required for TFMConv1D. Please install it with: pip install transformers")
